...
std::shared_ptr<CPicture> CPictureFactory::Create()
{
    ...
    // Create and both machines 
    CMachineActorFactory mfactory;
    auto machineA = mfactory.Create();

    machineA->SetPosition(Point(0, 0));
    picture->AddActor(machineA);

    // Create and both machines 
    CMachineActorFactory2 mfactory2;
    auto machineB = mfactory2.Create();


    machineB->SetPosition(Point(0, 0));
